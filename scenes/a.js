function A() {
  ...
  this.setup = () => {
    ...
    for (let i = 0; i < this.sceneManager.amount; i++) {
      this.strokeColors.push(
        color(random(0, 30), random(0, 120), random(0, 170))
      );
  ...
  this.draw = () => {
    ...
    if (this.introText) {
      ...
      textAlign(CENTER);
      text(
        "Turn up your volume to experience the sounds of coral...",
        width * 0.5,
        height * 0.4
      );
    ...
    if (this.demoText) {
      ...
      textSize(16);
      text("June 8th at 5:30 PM PT / 8:30 ET", width * 0.5, height * 0.4 + 30);
      pop();
    }
    //variables
    ...
    this.sceneManager.coral.onHover(
      (s) => {
        if (s.velocity.x !== 0) {
          s.velocity.x = 0;
        }
  ...
  this.mousePress = () => {
    ...
    if (this.currentLine >= this.sceneManager.textA.length) {
      ...
      this.currentSprite = this.sceneManager.coral.coralTips.get(
        this.currentLine
      ); //set currentSprite according to the line
