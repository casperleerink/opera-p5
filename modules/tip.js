...
class Tip {
  ...
  get bezierDeviation() {
    return this._bezierDeviation;
  }
  set bezierDeviation(f) {
    this._bezierDeviation = f;
  }

  bounceBoundaries() {
    if (
      this._pos.x >= this._boundaries[1] ||
      this._pos.x <= this._boundaries[0]
    ) {
      this._baseVel *= -1.0;
    }
  }
  ...
  drawC(p, base, xDeviation) {
    p.bezier(
      base.x * p.width,
      base.y * p.height,
      ((this._pos.x - base.x) * 0.1 +
        base.x +
        this._bezierDeviation +
        xDeviation) *
        p.width,
      ((base.y - this._pos.y) * 0.9 + this._pos.y) * p.height,
      ((this._pos.x - base.x) * 0.6 +
        base.x +
        this._bezierDeviation +
        xDeviation) *
        p.width,
      ((base.y - this._pos.y) * 0.15 + this._pos.y) * p.height,
      (this._pos.x + xDeviation * 0.3) * p.width,
      this._pos.y * p.height
    );
  }
}
